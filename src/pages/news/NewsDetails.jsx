...
class NewsDetails extends React.Component {
  ...
  getList=() => {
    ...
    console.log(value)
    axios.post('/applicaion/newsdetailget', {
      value
    }).then(item => {
      // this.setState({
      //   newData: item.data
              ...
              <Row>
                <Col span={14}>
                  <div className='details-right-div'>
                    <p className='details-right-title'>{(!_.isEmpty(this.newData)) && this.newsData.news_title}</p>{/* this.state.newsData.news_title */}
                    <span className='details-right-time'>发布时间:{(!_.isEmpty(this.state.newData)) && this.state.newsData.news_time}</span>{/* this.state.newsData.news_time */}
                    <div className='details-right-div-div'>
                      {(!_.isEmpty(this.state.newData)) && this.state.newsData.news_desc}{/* this.state.newsData.news_desc */}
                    </div>
                  </div>
                </Col>
              </Row>
